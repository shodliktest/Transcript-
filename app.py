...
# --- 2. SESSION STATE (Holatni saqlash) ---
if 'page' not in st.session_state:
    st.session_state.page = 'upload' # Boshlanishida 'upload' sahifasi
if 'player_html' not in st.session_state:
    st.session_state.player_html = ""

# --- 3. STEALTH MODE & DIZAYN ---
st.markdown("""
    <style>
        /* Streamlitni yashirish */
        #MainMenu, footer, header, .stDeployButton, [data-testid="stStatusWidget"], 
        [data-testid="stHeader"], [data-testid="stToolbar"] {visibility: hidden; display: none;}
...
# --- 4. ASOSIY MANTIQ (SAHIFA ALMASHINISH) ---

# ==========================================
# 1-SAHIFA: YUKLASH (UPLOAD PAGE)
# ==========================================
                ...
                try:
                    ...
                    for i, item in enumerate(all_words_with_time):
                        ...
                        if i > 0 and clean_word and clean_word[0].isupper():
                            ...
                            tr = GoogleTranslator(source='auto', target=t_code).translate(full_text) if t_code and t_code else None
                    ...
                    time.sleep(0.5)
                    my_bar.empty() # Bar'ni o'chirish

                    # HTML ni tayyorlash va Sessiyaga saqlash
                    audio_b64 = base64.b64encode(uploaded_file.getvalue()).decode()
                    json_data = json.dumps(final_lines)
                    
                    st.session_state.player_html = f"""
                    ...
                    st.rerun() # Sahifani yangilaydi (Upload qismi yo'qoladi)

                except Exception as e:
                    st.error(f"Xatolik: {e}")

# ==========================================
# 2-SAHIFA: PLAYER (PLAYER PAGE)
# ==========================================
elif st.session_state.page == 'player':
    
    # Player HTML
    components.html(st.session_state.player_html, height=700)
